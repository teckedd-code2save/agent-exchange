...
export default async function TransactionsPage() {
  ...
  const payload = await apiGet<{ results: Call[] }>('/api/v1/provider/calls');
    ...
    <main className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-400">
          Every proxied call recorded here — challenge loops, response status, latency, and environment.
        </p>
      </div>
      ...
      {calls.length === 0 ? (
        <div className="rounded-[2rem] border border-dashed border-slate-800 bg-slate-900/40 p-12 text-center">
          ...
          <p className="mt-3 text-sm text-slate-400">Hit one of your sandbox endpoints to start building a transaction log.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-slate-800">
              {calls.map((call) => (
                <tr key={call.id} className="align-top">
                  ...
                  <td className="px-5 py-4 text-slate-300">{call.amount} {call.currency}</td>
